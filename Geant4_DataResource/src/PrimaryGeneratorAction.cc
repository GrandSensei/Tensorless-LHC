...
// Ensure we only start one listener
...
//         std::cout << "❌ Listener failed to bind port 5003" << std::endl;
//         return;
//     }
...
//                             }
//                         }
//                         std::cout << "📥 Buffered " << count << " events." << std::endl;
//                     }
//                 }
//             }
//             close(clientSocket);
...
PrimaryGeneratorAction::~PrimaryGeneratorAction() {
    ...
    while (!hasData) {
        {
            // Lock the global mutex from CommandQueue.hh
            std::lock_guard<std::mutex> lock(g_queueMutex);
    ...
    G4ParticleDefinition* particle = particleTable->FindParticle(cmd.particleType);
    ...
    G4double zPos = worldBox ? -worldBox->GetZHalfLength() : -10.0*cm;
